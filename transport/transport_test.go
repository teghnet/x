...
func TestTransportDoesNotMutateCallersRequest(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		srv := httptest.NewTestServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
		}))
...
func TestTransportMutatorReappliedPerAttempt(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		var seen []string
		srv := httptest.NewTestServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if seen[0] == seen[1] || seen[1] == seen[2] {
			t.Errorf("mutator was not re-applied per attempt: %v", seen)
		}
	})
}
